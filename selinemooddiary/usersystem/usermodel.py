...
class DiaryUserManager(BaseUserManager):
    ...
    def create_user(self, email, username="Друг", password=None, birthday=None, **extra_fields):
        if not email:
        ...
        selin = None
        user = self.model(email=email, username=username, birthday=birthday, selin=selin, **extra_fields)
        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, email, username="Друг", password=None, birthday=None, **extra_fields):
